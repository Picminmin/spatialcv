...
def spatial_train_test_split_v2(
    ...
    # 動的グリッド
    nr, nc = cfg.n_rows, cfg.n_cols
    if cfg.dynamic_grid:
        if target_train_ratio < 0.3 or target_train_ratio > 0.7:
            nr = min(cfg.max_rows, max(cfg.min_rows, cfg.n_rows*2))
            nc = min(cfg.max_cols, max(cfg.min_cols, cfg.n_cols*2))
    ...
    train_blocks = _grow_connected_blocks(counts, nr, nc, need_train, target_train_pixels, cfg.ratio_tol, rng)
    if train_blocks is None or len(train_blocks)==0:
        raise RuntimeError("train ブロックの連結選択に失敗しました。パラメータを見直してください。")
    train_mask = _mask_from_blocks(block_id, train_blocks)
    train_mask &= (y != cfg.background_label)

    # ========== 2) test 選択 (train 以外から連結) ==========
    remain_blocks = set(range(n_blocks)) - set(train_blocks)
    if len(remain_blocks)==0:
        raise RuntimeError("train が全域を占有しています。test を作れません。")

    # test の目標
    target_test_ratio = test_size
    target_test_pixels = int(target_test_ratio * total_labeled)

    # train を除外した counts を用意
    ...
    test_blocks = _grow_connected_blocks(counts_remain, nr, nc, need_test, target_test_pixels, cfg.ratio_tol, rng)
    if test_blocks is None or len(test_blocks)==0:
        raise RuntimeError("test ブロックの連結選択に失敗しました。min_test_per_class や test_size を見直してください。")
    # train と重複しない保証
    test_blocks -= set(train_blocks)
    test_mask = _mask_from_blocks(block_id, test_blocks)
    ...
    if val_size and val_size > 0.0 and cfg.min_val_per_class > 0:
        ...
        # 残りブロック中、cand 画素があるブロックだけで counts を作る
        ...
        val_blocks = _grow_connected_blocks(counts_cand, nr, nc, need_val, target_val_pixels, cfg.ratio_tol, rng)
        if val_blocks:
            # 既存と排他
            val_blocks -= (set(train_blocks)) | set(test_blocks)
            val_mask = _mask_from_blocks(block_id, val_blocks)
            val_mask &= cand

    # ========== 4) unlabeled (train の輪帯, test/val/背景を除外) ==========
    background_mask = (y == cfg.background_label)
    # guard: inductive を担保するために U はtest/val と重ならない
    target_u = None
    if cfg.unlabeled_multiplier is not None:
        target_u = int(np.ceiol(cfg.unlabeled_multiplier * train_mask.sum()))
    if cfg.unlabeled_count is not None:
        target_u = cfg.unlabeled_count if target_u is None else min(target_u, cfg.unlabeled_count)
    if target_u is None:
        target_u = int(np.ceil(1.0 * train_mask.sum())) # 既定 : 同数
